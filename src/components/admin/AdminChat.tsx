...
export default function AdminChat() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-emerald-800">B2B Chat System</h1>
        <p className="text-sm text-emerald-800/50">{chatRooms.reduce((s, r) => s + r.unreadCount, 0)} unread messages</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-220px)]">
        {/* Chat List */}
        <GlassCard className="overflow-hidden flex flex-col">
          <div className="p-4 border-b border-emerald-200/20 space-y-3">
              ...
              {['all', 'unread', 'flagged', 'resolved'].map((f) => (
                <button
                  ...
                  className={cn(
                    ...
                    chatFilter === f ? 'bg-emerald-800 text-white' : 'text-emerald-800/40 hover:text-emerald-800'
                  )}
                >
                  {f}
                </button>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-medium text-emerald-800">{room.userName}</p>
                    <span className="text-[10px] text-emerald-800/30">
                      {new Date(room.lastMessageAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <p className="text-xs text-emerald-800/50 truncate mt-0.5">{room.lastMessage}</p>
        ...
        <div className="lg:col-span-2 flex flex-col">
          {activeRoom ? (
            <GlassCard className="flex-1 flex flex-col overflow-hidden">
              {/* Chat Header */}
              <div className="p-4 border-b border-emerald-200/20 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img src={activeRoom.userAvatar} alt="" className="w-10 h-10 rounded-full" />
                  <div>
                    <p className="text-sm font-medium text-emerald-800">{activeRoom.userName}</p>
                    <div className="flex items-center gap-1.5">
                      <span className={cn('w-1.5 h-1.5 rounded-full', activeRoom.isUserOnline ? 'bg-[#2ECC71]' : 'bg-emerald-800/20')} />
                      <span className="text-xs text-emerald-800/40">{activeRoom.isUserOnline ? 'Online' : 'Offline'}</span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button className="w-8 h-8 rounded-lg hover:bg-white/30 flex items-center justify-center">
                    <Flag className="w-4 h-4 text-emerald-800/30" />
                  </button>
                </div>
              </div>
                ...
                <AnimatePresence>
                  {roomMessages.map((msg) => (
                    ...
                    >
                      ...
                      >
                        <p className="text-sm text-emerald-800">{msg.content}</p>
                        <p className="text-[10px] text-emerald-800/30 mt-1">
              ...
              <div className="px-4 py-2 flex gap-2 overflow-x-auto scrollbar-hide">
                {quickReplies.map((reply) => (
                  <button
                    ...
                    className="flex-shrink-0 glass-pill px-3 py-1.5 text-xs text-emerald-800/60 hover:text-emerald-800 hover:bg-white/30 transition-colors"
                  >
                    {reply}
                  </button>
                ))}
              ...
              <div className="p-4 border-t border-emerald-200/20">
                <div className="flex items-center gap-2 glass-surface-strong rounded-2xl px-4 py-2">
                  <button className="w-8 h-8 flex items-center justify-center text-emerald-800/30">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-sm text-emerald-800 placeholder:text-emerald-800/30 outline-none"
                  />
                  <motion.button
                    whileTap={{ scale: 0.9 }}
                    onClick={handleSend}
                    ...
                    className={cn(
                      ...
                      messageText.trim() ? 'bg-emerald-800 text-white' : 'bg-emerald-800/10 text-emerald-800/30'
                    )}
                  >
            ...
            <GlassCard className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <MessageSquare className="w-12 h-12 text-emerald-800/20 mx-auto mb-3" />
                <p className="text-emerald-800/40">Select a conversation to start</p>
              </div>
            </GlassCard>
          )}
        </div>
