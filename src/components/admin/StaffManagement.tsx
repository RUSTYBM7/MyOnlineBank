...
const roleColors: Record<AdminRole, string> = {
  ...
  support_admin: 'bg-[#A8E6CF]/20 text-emerald-800',
  finance_admin: 'bg-[#DDA0DD]/20 text-emerald-800',
  viewer: 'bg-emerald-800/10 text-emerald-800/50',
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-emerald-800">Staff Management</h1>
          <p className="text-sm text-emerald-800/50">{adminUsers.length} admin accounts</p>
        </div>
            ...
            <GlassCard className="p-5">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-emerald-800">{admin.fullName}</p>
                    <p className="text-xs text-emerald-800/40">{admin.email}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-emerald-800/40">Last Login</span>
                  <span className="text-emerald-800">{new Date(admin.lastLogin).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-emerald-800/40">Status</span>
                  <div className="flex items-center gap-1.5">
                    <span className={cn('w-2 h-2 rounded-full', admin.isOnline ? 'bg-[#2ECC71]' : 'bg-emerald-800/20')} />
                    <span className="text-emerald-800">{admin.isOnline ? 'Online' : 'Offline'}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-emerald-800/40">IP Whitelist</span>
                  <span className="text-emerald-800">{admin.ipWhitelist.length} IPs</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 mt-4 pt-4 border-t border-white/10">
                <button className="flex-1 py-2 rounded-xl text-xs font-medium text-emerald-800/50 hover:text-emerald-800 hover:bg-white/20 transition-colors flex items-center justify-center gap-1">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-emerald-800">Add Admin</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs text-emerald-800/50 mb-2 block">Permissions</label>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          newAdmin.permissions.includes(perm.key)
                            ...
                            : 'glass-button text-emerald-800/50'
                        )}
                      >
                        {perm.label}
                      </button>
