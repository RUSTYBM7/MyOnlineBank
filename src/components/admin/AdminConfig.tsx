  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-emerald-800">Configuration</h1>
          <p className="text-sm text-emerald-800/50">Manage app settings and branding</p>
        </div>
        <GlassButton
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="font-medium text-emerald-800">Brand Settings</h2>
        </div>
        <div className="space-y-4">
          <div>
            <label className="text-xs text-emerald-800/50 mb-1.5 block">Brand Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-xs text-emerald-800/50 mb-2 block">Brand Colors</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={color.key}>
                  <label className="text-[10px] text-emerald-800/40 mb-1 block">{color.label}</label>
          ...
          <div>
            <label className="text-xs text-emerald-800/50 mb-1.5 block">Glass Opacity: {Math.round(localConfig.glassOpacity * 100)}%</label>
            <input
              type="range"
              min={0.05}
              max={0.5}
          ...
          <div>
            <label className="text-xs text-emerald-800/50 mb-1.5 block">Logo Upload</label>
            <div className="border-2 border-dashed border-white/30 rounded-2xl p-8 text-center hover:border-[#A8E6CF]/50 transition-colors cursor-pointer">
              <Upload className="w-8 h-8 text-emerald-800/20 mx-auto mb-2" />
              <p className="text-sm text-emerald-800/40">Click to upload logo</p>
              <p className="text-xs text-emerald-800/30">PNG, SVG up to 2MB</p>
            </div>
          </div>
        </div>
      </GlassCard>
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="font-medium text-emerald-800">Fee Structure</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={fee.key}>
              <label className="text-xs text-emerald-800/50 mb-1 block">{fee.label}</label>
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-emerald-800/30">{fee.suffix}</span>
              </div>
              <p className="text-[10px] text-emerald-800/30 mt-1">{fee.desc}</p>
            </div>
          ))}
        </div>
      </GlassCard>
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="font-medium text-emerald-800">Currency Management</h2>
        </div>
          ...
          {config.enabledCurrencies.map((curr) => (
            <div key={curr} className="flex items-center justify-between p-3 rounded-xl bg-white/30">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-[#A8E6CF]/20 flex items-center justify-center text-sm font-bold text-emerald-800">
                  {curr.slice(0, 1)}
                </div>
                <div>
                  <p className="text-sm font-medium text-emerald-800">{curr}</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-emerald-800/40">Active</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="font-medium text-emerald-800">Notifications</h2>
        </div>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/30">
            <div>
              <p className="text-sm font-medium text-emerald-800">Broadcast Message</p>
              <p className="text-xs text-emerald-800/40">Send to all users</p>
            </div>
            <GlassButton variant="primary" size="sm">Send</GlassButton>
          </div>
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/30">
            <div>
              <p className="text-sm font-medium text-emerald-800">Push Notifications</p>
              <p className="text-xs text-emerald-800/40">Firebase Cloud Messaging</p>
            </div>
